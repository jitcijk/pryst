...
public final class PrystMain {
    ...
    public static void main(String[] args) throws IOException, RuntimeException, MissingNameException {
        ...
        if (args.length == 0) {
            ...
            source = Source.newBuilder(new File(args[0])).mimeType(PrystLanguage.MIME_TYPE).build();
        }
    ...
    public static void executeSource(Source source, InputStream in, PrintStream out) {
        ...
        assert engine.getLanguages().containsKey(PrystLanguage.MIME_TYPE);
        ...
        try {
            ...
            } else if (cause instanceof PrystUndefinedNameException) {
                out.println(cause.getMessage());
            } else {
                ex.printStackTrace();
            }
